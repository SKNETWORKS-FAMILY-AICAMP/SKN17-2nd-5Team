...
- 범주형 변수와 이탈여부 간의 관련성을 확인하기 위해 카이제곱 검정, 크래머 V계수 
...
### 인사이트
현재 모델은 캘리포니아 지역 구독자 데이터를 기반으로 학습되었지만, 향후 한국 신문사 구독자 데이터를 확보·재학습함으로써 한국 시장 특성에 최적화된 예측 성능을 확보할 수 있습니다. 이 모델을 CRM(Customer Relationship Management) 시스템에 통합하면, 이탈 가능성이 높은 고위험 고객을 조기에 식별해서서 맞춤형 유지 전략을 신속히 적용할 수 있습니다. 이러한 접근은 단순한 기술적 고도화를 넘어,
- 구독자 유지율 향상
- 광고·구독 기반 수익의 장기적 안정화
- 데이터 기반의 경영 의사결정 지원
등의 실질적인 비즈니스 성과를 창출합니다. 
특히, 급변하는 미디어 환경 속에서 독자와의 관계를 강화하고, 장기적으로는 경쟁력 있는 비즈니스 모델을 유지하는 핵심 기반이 될 것입니다
...
<table>
  ...
  <tr>
    <td>한훈</td>
    <td>이번 프로젝트에서 가장 큰 도전은 대주제에 맞는 머신러닝 Task를 명확히 정의하고 적합한 데이터를 확보하는 일이었습니다. 소득과 나이처럼 서열형으로 제공된 변수를 논리적으로 타당하게 수치형으로 변환하는 전처리 과정도 쉽지 않았습니다. 그러나 이러한 과정을 거쳐 SMOTENC를 적용해 데이터 불균형 문제를 완화하고 모델 성능을 개선하는 데 성공했으며 그 성과에 큰 만족을 느꼈습니다.</td>
  </tr>
  <tr>
    <td>최동현</td>
